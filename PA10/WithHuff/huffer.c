...
int addWord(char * string, char * add, int max)
{
	//printf("addWord: (%s) to (%s)\n", add, string );
	//ensure there is enough memory in string to add add
	if ((strlen(string) + strlen(add)) >= max-3)
	{
		max *= 2;
	}
	string = strcat(string, add);
	return max;
}
...
char * deHuffer_string(char * huffed_string, WordData * huffman_tree)
{
	...
	int i;
	WordData * traverse = huffman_tree;
	for (i = 0; i < strlen(huffed_string); ++i)
	{
		if (huffed_string[i] == '\n' || huffed_string[i] == '\t')
		{
			char tmp[1];
			tmp[0] = huffed_string[i];
			tmp[1] = '\0';
			max_string_size = addWord(string, tmp, max_string_size);
		}
		else
		{
			//fputc(current, stderr);
			if (huffed_string[i] == '0')
			{
				//printf("read a 0\n");
				traverse = traverse->left;
				//found a leaf
				if (traverse->leaf == 1)
				{
					max_string_size = addWord(string, traverse->word, max_string_size);
					traverse = huffman_tree;
				}
			}
			else if (huffed_string[i] == '1')
			{
				//printf("read a 1\n");
				traverse = traverse->right;
				//found a leaf
